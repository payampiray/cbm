function cbm = cbm_hbi(data,models,fcbm_maps,fname,config,optimconfigs,isnull)
% cbm_hbi implements hierarchical Bayesian inference (HBI)
%       cbm = cbm_hbi(data,models,fcbm_maps,fname,config,optimconfigs,isnull)
...
% 7th input: 0 if for normal HBI and 1 for HBI under the null (optional)
...
input = struct('models',{models},'fcbm_maps',{fcbm_maps},...
...
initialize  = config.initialize; % inialization method
...
[qhquad,r,pmutau,pm,bound,prog] = cbm_hbi_init(cbm_map,hyper,isnull,initialize);

%--------------------------------------------------------------------------
% configuration of optimization procedure according to optimconfigs input, 
% see cbm_optim_config for more information

if length(optimconfigs)==1, optimconfigs = repmat(optimconfigs,K,1); end
if ~isempty(optimconfigs) && length(optimconfigs)~=K
...
for k=1:K
    d     = length(pmutau(k).a);
    optfigk = [];
    if ~isempty(optimconfigs)
        optfigk = optimconfigs(k);
    end
...
while ~terminate && iter<=maxiter
    ...
    math(iter)   = struct('qhquad',qhquad,...
                          ...
                          'input',input,'hyper',hyper); %#ok<AGROW>
...
[math.exceedance] = cbm_hbi_exceedance(math);
...
profile    = struct('datetime',datestr(now),'filename',mfilename,...
                    'config',config,'optimconfigs',optimconfigs,'hyper',hyper);    
